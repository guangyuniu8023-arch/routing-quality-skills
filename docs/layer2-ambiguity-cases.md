...
- **维度 1（输入类型）**：根据项目的输入模态划分。例如：
  - 媒体 Agent：无素材 / 单图 / 多图 / 图+视频 / 纯视频
  - 文本 Agent：纯文本 / 文本+链接 / 文本+数据
  - 宠物 Agent：纯文本 / 宠物照片 / 症状描述+照片

- **维度 2（意图类型）**：根据项目的 skill 意图空间划分。例如：
  - 媒体 Agent：创作 / 编辑 / 探索 / 反馈 / 澄清
  - 文本 Agent：查询 / 推荐 / 诊断 / 对比 / 闲聊

AI 从 Baseline Intent 的模糊区域推断适合当前项目的维度，呈现给用户确认后生成矩阵。

**AI 生成草稿：**
1. **模糊矩阵热力图草稿** — 基于维度 1 x 维度 2 交叉分析
...
向用户展示：
- 模糊矩阵热力图（基于 Baseline Intent 的模糊区域 + 维度 1 × 维度 2 交叉分析）
