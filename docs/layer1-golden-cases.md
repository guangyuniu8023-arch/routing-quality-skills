...
1. **AI 自动生成草稿** — 读取 Baseline Intent 的"必须通过的场景"，为每个 skill 生成 3-5 个 golden case 草稿（含 case_id, media, text, expected_skill, reason）。如果 config.md 已包含 Phase A 的必须通过场景，AI 以这些场景为种子，扩展生成更多 golden case（每 skill 扩到 5-8 个）。不重复已有场景。
...
将确认后的 golden cases 写入测试文件，运行测试。
